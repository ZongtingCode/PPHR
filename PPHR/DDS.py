...
class Stats:
    ...
    def ClickForResult(self):
        Patient_data = []
        # 输入校验，每条信息不能为空
        if self.ui.Albumin.displayText() == "":
            msgBox = QMessageBox()
            ...
            msgBox.exec()
        elif self.ui.Spesis.displayText() == "":
            msgBox = QMessageBox()
            msgBox.setWindowTitle('Tips')
            msgBox.setText("Please enter the Spesis!")
            msgBox.exec()
        elif self.ui.Reoperation.displayText() == "":
            msgBox = QMessageBox()
            msgBox.setWindowTitle('Tips')
            msgBox.setText("Please enter the Reoperation!")
            msgBox.exec()
        elif self.ui.PancreaticFistula.displayText() == "":
            msgBox = QMessageBox()
            msgBox.setWindowTitle('Tips')
            ...
            # 定义表头
            Patient_header = ['pralbum', 'prhct', 'optime', 'ASAclass', 'othsysep', 'returnor',
                              'pan_vascular_resection', 'CR_pan_fistula_2016v', 'othbleed']
            # 数据
            ...
            Patient_data.append(self.ui.Spesis.displayText())
            Patient_data.append(self.ui.Reoperation.displayText())
            Patient_data.append(self.ui.VascularResection.displayText())
            Patient_data.append(self.ui.PancreaticFistula.displayText())
            print(Patient_data)
            ...
            path = 'model/GBM_grid_1_AutoML_1_20240527_192928_model_2.zip'
            ...
            X = ['pralbum', 'prhct', 'optime', 'ASAclass', 'othsysep', 'returnor', 'pan_vascular_resection', 'CR_pan_fistula_2016v']
